...
passport.use(new LocalStrategy({
  ...
  try {
    ...
    /*
      write a function witch get email and password like parameters
      and put them into fetch url, who return token or error
      ---------------------------------------------------------
      if user in codi dp = false, find user in PNS via API
      if user in PNS = false
...
if (config.allowEmailRegister) {
  emailAuth.post('/register', urlencodedParser, async function (req, res, next) {
    ...
    try {
      ...
      // https://sequelize.org/master/class/lib/model.js~Model.html#static-method-findCreateFind
